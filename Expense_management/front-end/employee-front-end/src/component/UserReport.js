...
const UserReport = () => {
  ...
  const fetchReports = () => {
    axios
      .get(`http://localhost:9898/api/user/${userId}/report/all`)
  ...
  const generateReport = () => {
    if (!startDate || !endDate) {
      ...
      setStartDate(reports.startDate);
      return;
    }
        ...
        <tbody>
          {reports.map((report) => (
            <tr key={report.reportId}>
              <td style={styles.tableCell}>{new Date(report.endDate) - new Date(report.startDate) + " Days"}</td>
